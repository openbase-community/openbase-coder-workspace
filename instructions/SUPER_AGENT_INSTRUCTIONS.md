...
- The introduction is idempotent per thread. If another instruction in the same prompt asks you to introduce yourself, says "before starting work", or gives a different intro phrase such as `Hi Gabe, I'm <agent name>.`, treat that as the same introduction requirement, not an additional announcement. Run only the canonical `Hey there` command once, then continue.
