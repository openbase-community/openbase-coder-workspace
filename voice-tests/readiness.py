...
def ios_ready(source: str, *, state: str = "Listening") -> bool:
    tree = ET.fromstring(source)
    ...
    listening = any(state in e.get("label", "") for e in visible)
...
def valid_permit(permit: dict, nonce: str, now_ms: float | None = None, *, phone_state: str = "listening") -> bool:
    ...
    return (permit.get("nonce") == nonce and permit.get("allow") is True
        and permit.get("microphone_enabled") is True and permit.get("phone_state") == phone_state
