...
def events(directory: Path) -> list[dict]:
    ...
    for record in list(read_jsonl(directory / "ios.jsonl")) + list(read_jsonl(directory / "ios-upload.jsonl")):
        ...
        if not stamp:
            continue
        callback = metadata.get('callback_unix_ms') if message == 'local audio capture callback' else None
        rows.append({"source": "ios", "unix_ms": float(callback) if callback is not None else unix_ms(stamp), "event": metadata.get("event", message),
