...
`voice-tests/capture.py` reuses the shared Cartesia/AssemblyAI acoustic probe. A scenario JSON contains `seconds` and ordered `stimuli` entries with `at_s` and `text`; offsets are scheduled from recorder readiness. Use enough capture time for the final response and announcements. A scenario schedule is instrumentation, not a replacement for agent-driven observation or Appium phone control.
