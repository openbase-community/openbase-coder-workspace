...
- **Swift networking menu bar (optional).** Run `./scripts/dev-launch --menu-bar` for native connection/status feedback, or `./scripts/dev-launch --all` for it plus Electron. Both have matching VS Code tasks; the Electron/React surface launches from `tasks.json`. The menu bar app must be the signed staged build at `desktop/companion-build/OpenbaseNetmesh.app` (dev-launch stages it via `companion:stage:netmesh-menubar`); the desktop app's ensure loop terminates adhoc-signed builds because NetmeshHelper validates client code signatures. Electron also relaunches the staged menu bar automatically on a 60s timer, so it usually recovers on its own once the staged app exists.
...
- **Agent profiles:** Openbase shares authentication and history with the normal agent homes, but applies its settings and MCP servers through session profiles. Codex clients load `~/.codex/openbase.config.toml` through per-thread API config; Claude uses the settings and MCP files under `~/.openbase/profiles/claude/`. Codex file hooks are for native `codex -p openbase` commands; app-server sessions receive identity from the client. The shared Codex daemon has no global Openbase model/provider overrides. By default setup keeps only these profiles and *strips* the Super Agents MCP from the default (non-Openbase) `~/.codex/config.toml` and `~/.claude.json`, so plain terminal `codex`/`claude` sessions don't load it. Pass `openbase-coder setup --shared-super-agents-mcp` to instead register (add-if-absent, never clobbering a user's own entry) and keep it in those default homes — it defaults on for UI-driven desktop installs (`--json-progress`) and off for developer terminal setup. Existing installs can run `openbase-coder profiles install` and restart services. Openbase instruction files render into `~/.openbase/instructions/` from the workspace `instructions/` templates.
...
To verify a host after updating develop, check its standing branches against freshly fetched remote refs, preserve other contributors' uncommitted work, and verify the running installation rather than relying on checkout revisions. Refresh editable CLI build metadata with `uv sync --frozen --extra dev --reinstall-package openbase-coder` when its displayed version still identifies an older revision. Force `pnpm --dir console build` and `pnpm --dir desktop build` after shared renderer changes; the developer launcher's timestamp check alone does not cover changes in sibling renderer packages. Quit the old desktop instance before relaunching the developer dashboard, and verify that the live UI loads the workspace renderer and reports the refreshed CLI version. Restart LiveKit server as well as its dependent agent when runner configuration changes; an agent-only restart leaves the old engine configuration running. Require successful doctor checks, API health and visible service status after the services settle. A checkout with preserved local changes must be described as develop plus local changes, not as a clean develop build.
...
## Developer runtime freshness

The desktop and web dashboard show a top banner when running services or loaded UI assets differ from their configured checkout commits. The warning stays on one row. Hover over it or select **Details** to see each stale or unverified component and its refresh instructions in a scrollable popover. This is a commit-level check: uncommitted edits, already-running agent-session prompts, and mobile client source are not verified. Native companions without process provenance are explicitly marked unverified rather than assumed current. The check is advisory and never restarts calls or agents.

After first installing freshness instrumentation, restart managed services and rebuild/relaunch the developer desktop once to establish startup/build stamps. A missing stamp remains unverified until the component is refreshed. Python services capture CLI and editable Super Agents revisions at startup; desktop and console builds capture shared UI dependency revisions. Rebuilding a renderer does not refresh an already-loaded window. The developer launcher checks dependency commit stamps as well as source timestamps before reusing a renderer build. Production desktop and standalone runtimes do not run source freshness checks.

The native VPN components use a signed build manifest bound to each linked Mach-O image UUID. The app and companion publish frozen per-process startup records; `netmesh-ctl provenance` reads the helper's frozen record over mutually authenticated XPC. The checker verifies PID/start time, loaded image identity, workspace identity, and the `netmesh-macos` commit. Replacing a bundle on disk cannot refresh an already-running old process. Missing stamps, old helpers, failed XPC probes, and builds without the private source checkout remain unverified rather than being inferred current from an app version.

Build updated developer bundles with `pnpm --dir desktop companion:stage:netmesh` and `pnpm --dir desktop companion:stage:netmesh-menubar`. Staging captures source identity before compilation, checks it again afterward, stamps the linked app/helper images, and signs the manifest with the bundle. Use the existing companion helper-replacement flow and relaunch the native apps to load them; do not weaken XPC signing or manipulate root VPN state to satisfy freshness. Bare Xcode builds have no staging manifest and remain unverified. Screen-sharing companions and mobile clients are not yet source-verified.
