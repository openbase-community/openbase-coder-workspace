...
If setup selected Openbase Direct before an Openbase login existed, setup installs the embedded service without treating its expected `NeedsLogin` state as a failure. A successful `openbase-coder login` then mints an enrollment key, saves the matching staging or production control URL, connects Direct, restarts the transport-dependent services, and re-registers the computer automatically. No second transport command should be necessary. If that automatic step warns, run `openbase-coder tailnet enroll`; an `invalid pre auth key` error means the installed `OPENBASE_TSNET_CONTROL_URL` does not match the Cloud environment that minted the key, not that the account password is wrong.
...
- **Android app.** The Kotlin/Compose peer of the iOS app. Sign the phone into the same Openbase Cloud account, choose the computer during onboarding, and let the selected Openbase VPN or Direct transport supply the private backend address. Settings → Backend Host is an Advanced recovery/diagnostic control, not a normal onboarding step (see `android/README.md`).
