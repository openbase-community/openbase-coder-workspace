...
- **Attaching a Codex TUI to the shared daemon:** always pass the endpoint explicitly — `codex --remote unix:// …`. Do not rely on Codex's implicit local-daemon discovery: if the daemon is unreachable it silently falls back to a private embedded server, so the session looks fine but is invisible to the dispatcher, Super Agents, and the phone apps, and it collides with the daemon's thread ownership later. Explicit `--remote` fails loudly instead. One consequence: Codex rejects permission-override flags (approval-policy or sandbox overrides) on `resume`/`fork` against an explicit remote endpoint — the resumed thread keeps the permission settings it was started with on the server — so drop any such flags when resuming (nothing is lost). Name-based resume additionally requires the daemon's active cli/vscode thread listing to fit one server page — see `TROUBLESHOOTING.md` ("Cannot verify a unique session label").
