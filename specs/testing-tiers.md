...
- Core field-test account lifecycle: real product signup with a fresh `delivered+openbase-field-<opaque-run-slug>@resend.dev` address in Openbase's reserved Resend testing namespace, real allauth message rendering and Resend submission through the selected Cloud deployment, retrieval through a dedicated secure Resend CLI profile, real confirmation through the tested product, optional post-verification local entitlement, and canonical teardown. No per-address deployment allowlist is required. The `field_test_account` command in openbase-drf-api-core can destroy or grant entitlement but cannot create or verify users. Personal inboxes are forbidden.
