...
- **Give an audible notice before foregrounding Tart.** Whenever a field-test step requires bringing the Tart window to the foreground for a visible guest interaction, first run `tts "Hey Gabe, I need to foreground the disposable Tart test VM for the next field-test step."`, then foreground Tart and perform the interaction yourself. This notice is informational, not a request for permission or a handoff of the VM step. Do not emit it while the phone may be listening during an acoustic test; in that case, end or pause the listening state before the notice, then restore the intended test state afterward.
