...
1. Create a new folder with a short, speech-friendly name and a `briefing.md` that asks for a small exact file-backed task. Avoid words the configured stimulus voice pronounces ambiguously; for example, use numbered folders instead of “retry.” Tell the agent to announce completion with `openbase-coder user say`, not to “report to” or “return to” the dispatcher, because those phrases can be misread as a voice-route command. On macOS/Tart, put the folder on the VM Desktop and reset only Desktop-folder TCC in the disposable guest before the attempt. Do not accept an earlier grant from onboarding or another test in the same VM as coverage of this gate.
