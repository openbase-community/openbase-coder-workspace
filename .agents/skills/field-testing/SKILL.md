...
0. **Front-load EVERY human action at run start (Gabe's rule, 2026-09-12) — and "front-load" means EXECUTE the prompt-triggering steps first, not merely announce them.** Announcing the asks and then spending 15 minutes provisioning a VM still lands the trust/passcode prompt late. Before touching the VM, ask the user to unlock the physical test phone, keep it next to the Mac speakers/on a charger, and prepare the platform-specific human gates. On iOS, set Auto-Lock to Never and expect one or two device-passcode prompts for field-test VPN configuration and Mac trust. On Android over USB, enable USB debugging and have the user accept the computer/RSA trust prompt. The agent never learns or enters a device passcode. Then immediately establish Appium control and drive the phone's user-gated chain (iOS: unlock → WDA → field-test app sign-in → VPN connect → device-passcode prompt; Android: USB trust → field-test app install/sign-in → VPN connect and any system VPN confirmation) in the first minutes, before machine-only long-lead work. Keep that single Appium session attached through all acoustic testing, follow-up requests, audit completion, and the final audible handoff; a passing assertion is not permission to release device control while the agent is still working. Warn that the field-test VPN displaces the normal app's VPN because only one can be active. New human dependencies discovered mid-run get asked immediately and appended here for future runs.
