...
The managed Openbase Cloud path is Overview → Prerequisites → Setup → Sign in → Phone → Pairing → Verify. It has one desktop authentication stage: the Openbase account sign-in also activates managed coding and managed voice. Choosing BYO Codex or Claude Code adds exactly one Agent sign-in stage. Choosing Cartesia adds Voice Keys; managed Cloud and local audio do not. The phone still signs in to the same Openbase account on that separate device, but it is not another desktop provider-auth stage. The transport choice does not add an authentication stage: Openbase VPN may add macOS helper approval and an iOS VPN-configuration passcode prompt. Openbase Direct adds neither on the computer or iOS; until Android ships the in-process Direct transport, Android joins that same Openbase-managed network through its bundled Openbase VPN and may show the normal Android VPN-consent dialog. This is a platform transport detail, not Tailscale authentication: the Android UI must explain it and must never direct a managed-network user to install Tailscale or choose an alternate server.

## Hard Boundary Zero: A Field Test Starts From A Fresh Clone, Every Time

**A run only counts as a field test if it begins with `tart clone` of a maintained source image in that same run.** A VM that already exists — left over from a prior session, already provisioned, already signed in, already paired — is **contaminated evidence** for a new clean-room run. Keep the just-tested disposable VM after the traditional field test so the user can request immediate targeted follow-ups against the exact observed state; clearly label those follow-ups as debugging rather than another field-test pass. Only when a later request actually needs a new clone, begin by running `tart list` and deleting one or more stale disposable field-test clones, while preserving maintained golden/source images and any VM still needed for active evidence; then create the fresh clone. Continuing on a retained VM and reporting the result as a new "field test" is a process failure, not a judgment call (real incident, 2026-09-13: an agent resumed a warm previous-session VM whose checkout was on `staging` with hand-copied patches, ran the acoustic loop on it, and reported the developer-install "field test" phase complete — it had tested nothing about installation, setup, onboarding, or first-run state, which is most of what field tests exist to catch).
...
- **Do not fight macOS App Management to patch a signed app during retained-VM debugging.** Tart Guest Agent may be prevented from modifying `/Applications/Openbase.app` even with passwordless `sudo`. Keep the signed bundle intact: copy the bundled `openbase_coder_cli` package to an explicit debug overlay such as `~/.openbase/field-test-overlay`, point only the relevant retained VM launchd wrapper at it with `PYTHONPATH`, and restart that service. Record the deviation and never count the result as clean-room evidence; the next actual field test must install the rebuilt artifact normally.
...
- **Host OS runs in a disposable VM.** On macOS, use a [Tart](https://tart.run) macOS VM exclusively — the harness in `install-tests/electron-macos/` clones a throwaway VM so the real machine's install, launchd services, Tailscale routes, and ports 7999/7880 are never touched. The native-Windows pathway uses a Windows VM (later). Never install a field-test build onto the developer's own macOS session.
  - **A fresh clone must also be clean at the product layer.** A maintained Tart source image may contain OS updates and generic test prerequisites, but it must not carry `~/.openbase`, an Openbase app in `/Applications`, an `openbase-coder` shim, Openbase launch agents, Electron user data, a registered `cloud.openbase.netmesh.helper`, a Cloud login, or a prior field-test account. Immediately after cloning and before creating the account or installing Openbase, check those surfaces. If any are present, remove them through the documented uninstall flow (including unregistering the VPN helper) and re-check before proceeding; also clean or replace the contaminated maintained source image before the next run. A clone is not fresh merely because its Tart name is new.
...
1. Create a new folder with a short, speech-friendly name and a `briefing.md` that asks for a small exact file-backed task. Prefer a single ordinary word made only of letters for both the folder and thread name; speech recognition can normalize a spoken multiword name differently from an on-disk hyphenated name, causing a false missing-folder clarification. Avoid words the configured stimulus voice pronounces ambiguously; for example, use a folder such as `Maple` instead of “retry.” Tell the agent to announce completion with `openbase-coder user say`, not to “report to” or “return to” the dispatcher, because those phrases can be misread as a voice-route command. On macOS/Tart, put the folder on the VM Desktop and reset only Desktop-folder TCC in the disposable guest before the attempt. Do not accept an earlier grant from onboarding or another test in the same VM as coverage of this gate.
2. By voice, tell the dispatcher to start a coding session in that folder and follow the briefing. Do not request a person/speaking name: the dispatcher chooses the durable thread name first and the speaking name must always be the deterministic name corresponding to that exact thread name. Precompute that name with `openbase-coder super-agent-name "<thread name>" --json` only when the briefing needs it for a completion command, then assert the stored `agent_name` matches the command result. Keep brittle specifics in the briefing and do not tell the Super Agent to introduce itself; the announcement must be unsolicited.
...
Note the store's `UNIQUE(name)` on sessions is not backend-scoped: if a prior run left a `dispatcher` (or same-named) session under a *different* backend identity, `create_session` collides (`sqlite3.IntegrityError: UNIQUE constraint failed: sessions.name`). Start from a fresh clone. For a dedicated disposable preparation VM, stop all writers and reversibly archive the fixture-owned managed store, provenance registry and effective backend history together, following the accelerated voice runbook. Never repair this by deleting individual session rows: related turn state and indexed backend history can survive or be reimported. Preserve the old evidence and never reset a personal installation.
...
- **Inbound capture:** the phone's spoken reply recorded back through the host microphone and transcribed by the bundled `scripts/acoustic-probe.py` (resolved relative to this skill directory, not the workspace root), so the agent asserts on the *meaning* of what the product said, not just on log lines. When verifying that helper exists, use `git ls-files '.agents/skills/field-testing/scripts/*'` or an explicit path; a default `rg --files` from the workspace root skips hidden `.agents/` paths. That host-side transcription uses AssemblyAI (`$ASSEMBLYAI_API_KEY`, `--stt mlx` local fallback) — Openbase Cloud STT is realtime-streaming for live calls, not a batch-file endpoint, so it is not the tool for transcribing a saved recording.
...
- **Query system alerts immediately after every first `call.start` on a fresh app install.** Call `appium_alert action=get_text` before interpreting a page source that says `Connecting...`: iOS can present the microphone permission sheet while the ordinary accessibility tree still describes the underlying call UI. If the sheet says the field-test app would like microphone access, accept **Allow** through Appium, then verify it closed. Current app code requests and resolves microphone permission before joining; Android must automatically continue the pending Start action after `RECORD_AUDIO` is granted, so an accepted permission sheet that returns to `ROOM disconnected / CALL idle` is a regression, not a cue for the tester to tap Start twice. If an older build already joined a room before permission was granted, end that stale room and start a fresh call. Microphone permission is not the VPN passcode prompt, and the agent can accept it without asking the user to touch the phone.
- **Reconcile transient agent-presence events against the current room before alarming the user.** A brief participant disconnect during LiveKit negotiation or reconnection may set an agent-left warning, but the warning must clear as soon as any current agent participant is present; `lk.agent.state=listening` or another ready state alongside `The agent left the call` is a regression. Clear only presence-derived warnings so actionable status notices such as a Desktop-permission blocker survive. Reconnecting is already the primary call subtitle, so do not add a second generic `Network unstable` label or banner.
- **Require stable Android media, not signaling-only success, on Openbase Direct.** Direct keeps the computer's LiveKit server loopback-only and publishes authenticated `voice_turn` credentials from `/api/onboarding/status/`. Because Android joins the managed network with its bundled VPN, its LiveKit client must dial the computer's TURN endpoint over that VPN and force relay media, matching the VPN branch of the iOS behavior. A momentary `ROOM connected` followed by perpetual `Reconnecting…`, failed ICE candidate pairs, or an agent receiving no microphone audio is a field-test failure even if signaling and the agent participant briefly connected. Never “fix” this by exposing LiveKit on the LAN or hardcoding TURN credentials.
...
- **Verify automatic unmute against the current lifecycle protocol before disabling it for a probe.** With Auto-mute and Auto-unmute enabled, `safe_to_mute_user` / `agent_audio_started` must mute and `safe_to_unmute` / `agent_audio_finished` must restore the mic after complete playback. A client that only handles the older `agent_audio_stopped` / `agent_idle` names remains stuck on `CALL muted` even though the backend emitted `safe_to_unmute`; that is a product regression, not an instruction to keep tapping Unmute. Disable Auto-mute only after recording the failed behavior when a clean acoustic probe must continue.
...
- **Check the host output route, too.** `system_profiler SPAudioDataType` must show the built-in speakers as the default output and the built-in microphone as the default input. Volume alone is not sufficient if the default route changed to a virtual device or headset.
...
- **Re-prove speaker mode after every Appium session recreation, app relaunch, and call restart.** Treat the audio route as call-local state that may reset even when the UI, bundle, and account were preserved. Before the first stimulus after any of those events, repeat the complete tap → screenshot → selected-fill verification above; evidence from an earlier call or Appium session is invalid. Write `SPEAKER_RECHECK=PASS` with the new screenshot timestamp in the daily field-test log before speaking.
...
- **Use one atomic spoken prompt and leave enough capture time for the answer.** Do not append a second sentence such as “Answer in one short sentence”: realtime STT may finalize it separately, steer/cancel the first turn, and create a false hang. The probe defaults to 45 seconds because the backend and complete phone playback can legitimately consume more than 20 seconds. Shorter windows that end as `voice_delivery_audio_delivered` appears do not prove the inbound acoustic leg.
...
- **When the dispatcher runs cloud-side there is no local `livekit-agent.log` to read** — a *managed* Cloud backend (openbase_cloud / openbase_cloud_codex, the fresh-install default) runs the dispatcher in Openbase Cloud, and a Mac paired via the **desktop app** doesn't write that log locally. In that case you cannot read the answer from a VM log; capture it acoustically instead with the bundled probe, which speaks the stimulus and transcribes the room (so the transcript holds *both* your question and the product's spoken reply):
  ...
  .agents/skills/field-testing/scripts/acoustic-probe.py "Why is the sky blue?"
  ```
  ...
  - **Microphone permission.** The default recorder uses native Core Audio through the bundled Swift helper; the terminal/app running it needs Privacy & Security → Microphone. Without permission the stimulus can still play while capture fails or contains no usable speech. Verify a retained recording with `ffprobe` and `ffmpeg -i out.wav -af volumedetect -f null -` before trusting an empty transcript.
  - **Do not switch the default back to FFmpeg AVFoundation because a device index looks convenient.** On affected macOS/FFmpeg combinations AVFoundation advances wall-clock timestamps while delivering only a fraction of the microphone samples. Padding those gaps produces a correctly sized but time-stretched, unintelligible file. The probe's native Core Audio default records the full sample stream; `--capture ffmpeg --device N` exists only as a diagnostic fallback.
...
- **Android:** from `android/`, first run `scripts/build-netmeshgo-aar.sh`, require the resulting untracked `app/libs/netmeshgo.aar`, then run `./gradlew :app:assembleFieldTest`. Before Gradle, require `$JAVA_HOME/bin/java`; on a host whose Homebrew JDK is installed but not registered with `/usr/libexec/java_home`, use `JAVA_HOME="$(brew --prefix openjdk@17)" ./gradlew …` instead of treating “Unable to locate a Java Runtime” as a product failure. Do not field-test an APK built through the compile-only netmesh stub: it cannot establish either Openbase-managed transport and creates a false onboarding failure. Install and launch the resulting field-test APK, and create the UiAutomator2 session against `com.openbase.android.fieldtest`. The variant has separate storage and the `openbase-field-test` authentication URL scheme, targets `https://app-staging.openbase.cloud`, and can coexist with `com.openbase.android`. Never substitute, reset, or uninstall the normal app. After connecting an Openbase-managed transport, require the connection card and first pairing-status row to agree: `Connected · 100.x` alongside `Waiting for the Openbase VPN on this phone` is a product regression. The bundled VPN service's published connected identity is authoritative for managed-network registration and readiness; OS-interface scanning belongs only to the expert Tailscale-app path. Also exercise one app relaunch or in-place field-test APK update while connected, including after onboarding is marked complete: the authenticated app shell must restore the managed network automatically instead of stranding the Call screen behind `Can't reach the Openbase backend.` A persisted Android node must resume without consuming another one-time enrollment key. An upgrade from a build that did not save its control URL may enroll once to seed that setting, and an actually dead stored identity may reset and re-enroll once, but recovery must not send the user to Tailscale, leave `Connecting…` indefinitely, or silently launch Android's system VPN-consent UI. Treat duplicate concurrent enrollment requests or overlapping VPN starts during one foreground transition as regressions: lifecycle and initial reconciliation must collapse into one in-flight attempt, and cancellation must not surface as a connection error. If consent is absent or revoked, leave reconnection user-initiated through onboarding or Settings.
- **Android startup warnings:** the authenticated shell starts its status monitor and may request an immediate refresh as settings hydrate. Overlapping failures must count as one observation, not satisfy the two-poll unreachable threshold instantly. During a normal in-place update or foreground recovery, do not show `Can't reach the Openbase backend` while the managed VPN resumes within a few seconds; a genuine outage still earns the warning after failures separated by the normal poll interval.
...
For an Openbase VPN run, the gate has not passed merely because the phone is authenticated, the VM's VPN is running, `Connect` was tapped, a notification sheet appeared, or the agent believes a prompt should be present. If iOS genuinely reuses an already-valid field-test VPN configuration and presents no add/modify passcode sheet, record `PASSCODE_GATE=NOT_PRESENT` with the observed Appium state and explain it immediately; never claim that the passcode was surfaced. Android follows the analogous earliest system VPN confirmation chain but has no iPhone-passcode assertion. Apply that Android confirmation chain to both current managed-network choices: Android's bundled client is a `VpnService` for Openbase VPN and for accounts whose computer uses Openbase Direct. If `VpnService.prepare` returns no confirmation because consent is already valid for the field-test package, record the observed connected state and continue without inventing a prompt.

1. Confirm the disposable VM path is ready (macOS): see `install-tests/electron-macos/README.md`. Before creating a new clone, inventory Tart and delete stale disposable field-test clones, preserving maintained source images and any just-tested VM retained for active follow-up evidence. For a full installation field test, use `manual-vm.sh` to start a bare VM and download the real signed channel DMG inside it; use a local unsigned app only when that developer build is explicitly the subject.
...
4.5. **Confirm the selected Cloud's netmesh is actually configured before starting an Openbase VPN or Direct pairing test.** Staging and prod each need their OWN headscale control plane; staging must NOT borrow prod's (prod's headscale API key is a global admin credential — the least-trusted env must never hold it). Verify from the selected Cloud app that `HEADSCALE_API_URL`/`HEADSCALE_API_KEY`/`HEADSCALE_CONTROL_URL` are set and the enroll endpoint returns 200 (not 502): `openbase run --memory 1024 -a "<Cloud app>" python -c "import os;print(os.environ.get('HEADSCALE_API_URL'))"`. Staging's isolated headscale is `net-staging.openbase.cloud`, provisioned by `../openbase-cloud-workspace/netmesh-infra/aws-headscale` with `-var-file=terraform.staging.tfvars` against `netmesh/headscale-staging.tfstate` (see that README's "Staging control plane" section). Direct must persist the `control_url` returned with its enrollment key before starting `openbase-tunneld`; a staging key submitted to the production default fails as `invalid pre auth key` and remains in `NeedsLogin`. If enroll 502s, netmesh is misconfigured — that is a real finding, and pairing/VPN/acoustic is blocked until it's fixed; do not wire staging into prod's headscale to force it. (Note: creating a *secret* config var via `openbase config set --secret[-stdin]` currently 500s; store non-DB secrets like `HEADSCALE_API_KEY` as a plaintext config var, matching how prod already stores it.)
