...
- **Start Safari computer control before opening OAuth.** On a fresh VM, run `guest-automate.sh enable-safaridriver <vm>` once, start `guest-automate.sh safari-tunnel <vm> 4444`, and create/prime the host WebDriver session before clicking the desktop app's login action. The login then opens in the Safari instance already under semantic control. Trying to attach only after the OAuth page opens is unreliable and led to an avoidable control dead end. Verify the address-bar origin is the expected staging or production origin before entering credentials; for a staging run, `app.openbase.cloud` is a hard stop.
- **Give Tart enough display space before boot.** Use `manual-vm.sh --display 1920x1200pt` (the default) or `tart set <vm> --display 1920x1200pt --no-display-refit` before the run. Synthetic scroll-wheel events, Page Down, and End are unreliable in Tart. A larger guest display makes the complete background-items list and onboarding controls visible without brittle resizing tricks.
...
- **A hung turn is often a GUI prompt in the VM — look at the Tart window.** Real case (2026-09-12): the dispatcher's Bash `mkdir ~/Desktop/...` hung indefinitely on macOS TCC — "python3.12 would like to access files in your Desktop folder" — with no timeout, no error, no spoken fallback (finding FT-9: agent turns touching Desktop/Documents/Downloads on a fresh install block on TCC with zero surfacing). When a turn stalls: screenshot the Tart window FIRST; if a TCC prompt is up, click Allow via host `cliclick` and the turn resumes. Durable prevention for future runs: pre-grant the service's folder access during VM provisioning, or have the first field-test smoke touch ~/Desktop deliberately to flush the prompt early. NOTE: EVERY protected folder is a separate TCC grant — Desktop and Documents prompt independently (both hit 2026-09-12), so flush BOTH (and Downloads) during provisioning. Also: a stall in a dispatcher-SPAWNED sub-agent is NOT caught by a watchdog that only wraps the dispatcher's own awaited turn — the dispatcher fire-and-forgets the sub-agent ("Grace is on it") and returns; the sub-agent then hangs in its own thread. Blocked-turn surfacing must cover spawned-agent threads, not just the dispatcher's. IMPLEMENTED in PR openbase#32 as a session-wide poller keyed on the RELIABLE signal — a turn failing with "Control request timeout: initialize" (a spawned process blocked at init behind a consent dialog). Two detectors that live-testing PROVED unreliable, do NOT reuse: (a) turn.status='running' past a threshold — blocked sub-agent turns flip to 'failed' fast and never dwell in 'running'; (b) dialog-presenter-process recency — UserNotificationCenter is long-lived and REUSED across dialogs, so "started during this turn" is false even with a dialog up. An on-screen CGWindowList owner probe (swift, no Screen-Recording needed for owner names) is the theoretically-cleaner live-dialog signal but couldn't be positively confirmed against a live prompt, so it was NOT shipped.
