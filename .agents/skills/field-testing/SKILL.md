...
- **Host OS runs in a disposable VM.** On macOS, use a [Tart](https://tart.run) macOS VM exclusively — the harness in `install-tests/electron-macos/` clones a throwaway VM so the real machine's install, launchd services, Tailscale routes, and ports 7999/7880 are never touched. The native-Windows pathway uses a Windows VM (later). Never install a field-test build onto the developer's own macOS session.
  - **Known Tart-VM limitation — the macOS Openbase VPN companion cannot connect in a VM.** The privileged `NetmeshHelper`↔`OpenbaseNetmeshCompanion` XPC handshake fails with `-67065 errSecCSGuestInvalid` (Virtualization.framework cannot resolve the XPC peer's guest code object), so `tailscaled` never starts and the Mac never joins the tailnet — even though both binaries pass `codesign --verify --deep --strict`. This is environmental, not a product bug (real hardware validates fine). Consequence: the **macOS Openbase VPN (netmesh) pairing + full-acoustic-loop leg is not exercisable in a Tart VM.** Do not burn time trying to force it. Run the VPN-companion/pairing/acoustic leg on **real Mac hardware**, or (once packaged) via **Openbase Direct** — but note Openbase Direct is currently broken on standalone/desktop installs because the release package omits `openbase-tunneld` (bin/ has only `livekit-server` + `openbase-coder`). The phone side (real device) DOES connect to netmesh normally; the block is Mac-in-VM only. Everything up to pairing (install, onboarding, setup, sign-in, phone enrollment) is fully testable in the VM.
