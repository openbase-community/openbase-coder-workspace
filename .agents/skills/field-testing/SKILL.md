...
The managed Openbase Cloud path is Overview → Prerequisites → Setup → Sign in → Phone → Pairing → Verify. It has one desktop authentication stage: the Openbase account sign-in also activates managed coding and managed voice. Choosing BYO Codex or Claude Code adds exactly one Agent sign-in stage. Choosing Cartesia adds Voice Keys; managed Cloud and local audio do not. The phone still signs in to the same Openbase account on that separate device, but it is not another desktop provider-auth stage.
...
- **SPEAKER mode is a hard precondition for speaking to the phone.** The required order is: connect the call → appium-tap `call.speaker` → take an Appium screenshot → verify the speaker control has the solid white/light selected background (the inactive control has a dark/navy background) → only then emit any host-TTS or other acoustic stimulus. A tap, an enabled control, the `call.speaker` identifier, and the `Volume High` accessibility label are not proof because they are present in both route states. If the screenshot does not prove the selected styling, do not speak; resolve the route or end and restart the call. iOS otherwise routes call audio to the earpiece, so the host-mic recording hears no reply even when `tts_stream_flush` and `voice_delivery_audio_delivered delivered=True` prove that the dispatcher generated and sent one. No screenshot-verified speaker mode = no acoustic test.
...
Follow the [conditional stage sequence](#conditional-desktop-onboarding-stages). For the managed path, click `Let's get you set up` → `recommended` (Openbase VPN) and `Check prerequisites` → answer `^No$` (no existing Codex/Claude CLI to import), click `Run setup`, then confirm with `I understand, run setup` → poll `curl 127.0.0.1:7999/api/health/` until 200 (setup installs the 5 launchd services and binds 7999; ~60–90 s) → click `^Continue$` → on Sign in, `Run login` opens browser OAuth. Drive it with `safari-tunnel` + `host-drive.mjs --wd`, verifying the origin is `app-staging.openbase.cloud` before filling the field-test credentials. The setup command is `openbase-coder setup --backend openbase-cloud --audio-provider openbase-cloud --tailnet-provider netmesh` (Cloud audio, as required). Do not expect or automate invisible placeholder stages for options the user did not choose.
