...
- **Deploy attribution:** PaaS Slack notifications attribute a deploy from the `Agent-Thread-Id` trailer on the pushed head commit. Both promote scripts stamp their synthesized exact-tree commits from `$AGENT_SESSION_ID` (Claude Code inject-session-id hook) falling back to `$CODEX_THREAD_ID` (exported natively by Codex shells) — the same chain the openbase CLI uses. If neither is in your environment, pass `--agent-thread-id <your-thread-id>` explicitly or the deploy shows up as `Agent: human`.
