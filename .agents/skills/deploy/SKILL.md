...
Watch the run and confirm the macOS log shows `Staged Openbase Coder CLI <version>`. Because the DMG seeds whatever CLI release exists when the build starts, the one-shot promote usually seeds the **previous** release — installed apps still converge to the new CLI through the update feed on next launch, so this is fine. (Bump `desktop/package.json` on develop before promoting when you want installed apps to auto-update — electron-updater only moves to *higher* versions.)

**Optional — pre-seed the DMG with the CLI you're releasing now.** If the first launch of a fresh install should already be on this release (e.g. a demo build), hold desktop back and promote it after step 3 confirms the CLI release exists: `./scripts/promote develop main --exclude desktop`, then `./scripts/promote develop main --skip-checks` to promote desktop alone.
