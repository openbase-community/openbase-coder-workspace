...
- **Deploy attribution:** PaaS Slack notifications attribute a deploy from the `Agent-Thread-Id` trailer on the pushed head commit. Both promote scripts stamp their synthesized exact-tree commits from `$AGENT_SESSION_ID`, the single vendor-neutral session variable agent runtimes export into agent shells (Claude Code via the inject-session-id hook, Codex via super-agents' per-thread `shell_environment_policy`). If it is not in your environment, pass `--agent-thread-id <your-thread-id>` explicitly or the deploy shows up as `Agent: human`.
