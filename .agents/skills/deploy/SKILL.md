...
- **Deploy attribution:** PaaS Slack notifications attribute a deploy from the `Agent-Thread-Id` trailer on the pushed head commit. Both promote scripts stamp their synthesized exact-tree commits from `$AGENT_SESSION_ID` (exported automatically in Claude Code sessions). If your session lacks that env var (e.g. the Codex backend has no env injection), pass your thread ID explicitly: `./scripts/promote develop main --agent-thread-id <your-thread-id>` — otherwise the deploy shows up as `Agent: human`.
