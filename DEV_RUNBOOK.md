...
- **Tests:** `cd cli && uv sync --extra dev --extra local-audio && uv run pytest`
  when this install uses local audio (omit the second extra for cloud audio;
  the venv lives at the workspace root — uv workspace); `cd super-agents && uv run pytest`;
