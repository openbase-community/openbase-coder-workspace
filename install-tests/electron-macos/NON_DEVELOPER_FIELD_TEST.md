...
- Every onboarding stage applicable to the selected setup completed using the bundled CLI and the sampled real transport.
- For Openbase VPN, the embedded Netmesh components matched the sampled release, survived one VM reboot, and restored VPN plus backend health without rerunning setup. For Openbase Direct, the bundled tunnel component was present and Direct restored tunnel plus backend health after one VM reboot.
...
- Require both Openbase VPN and Openbase Direct to be visible and require the VPN option to say it is Recommended, enables the full experience, and is built on Tailscale. Select the transport sampled in the run plan; a signed non-developer build must not offer the developer-only standalone Tailscale option.
...
- For Openbase VPN, approve the Netmesh background item through System Settings with Computer Use, following the shared disposable-VM rule, and keep setup running while macOS authorization completes. Openbase Direct must complete without that helper approval.
- Before pairing, apply shared Preflight checks 4.5–4.7, including the selected Cloud transport configuration, model availability, and the actual backend-health check on port 7999. Port 49154 alone proves only that the Electron control shell is running.

## 4. Verify the embedded transport components

For an Openbase VPN run, record both embedded Netmesh build numbers so a stale or wrong-channel prebuilt is visible:
...
For a staging DMG, require the builds expected by that staging release. Apply the shared post-approval readiness and concurrent-client checks; they are not repeated here. For an Openbase Direct run, instead verify the bundled standalone package contains `openbase-tunneld`, require the account transport and generated setup command to use `netmesh-tsnet`, and exercise the real tunnel without approving or launching the Netmesh background helper.

For an Openbase VPN upgrade test, launch the replacement app and compare `netmesh-ctl version` with the embedded build before rebooting. The already-onboarded desktop must reconcile the registered privileged helper at launch. If it does not, inspect bounded log tails and record the failure rather than rerunning setup or using `tailnet set-provider` to hide it:
