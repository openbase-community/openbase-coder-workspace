...
- `netmesh-go`: shared gomobile Tailscale-engine bridge (NetmeshGo) consumed by the iOS packet-tunnel extension and the Android VpnService (private, fixed to `main`, internal install set only)
...
- `openbase-agent-ui-swift` / `openbase-agent-ui-kotlin`: public dependency-light contracts (configuration, auth/secure-storage adapters, call-token preparation, lifecycle events, session-facing state) for embedding Openbase Agent UI in third-party iOS / Android apps; keep the two API surfaces aligned
...
- Licensing: public AGPL-3.0-only — `cli`, `console`, `coder-react`, `skills`, `multi-react`, `openbase-agent-ui-swift`, `openbase-agent-ui-kotlin`, `desktop` (Electron app; its vendored `companion/livekit-swift-example` screen-share companion stays upstream Apache-2.0). Public MIT exceptions — `allauth-client-swift`, `allauth-client-kotlin`, `super-agents`, `boilersync-react`. Private/proprietary — `android`, `ios`, `netmesh-go`, `netmesh-macos` (the netmesh companion, extracted from desktop 2026-09-03; internal install set, sibling checkout builds from source, public desktop checkouts use the signed prebuilt artifact — see `desktop/OPEN_SOURCE_BOUNDARY.md`). Desktop's public history begins at the 2026-09-03 fresh mirror; full pre-mirror history lives in the private `openbase-coder-desktop-history` repo. Never commit `netmesh-macos/` content into desktop (a pre-push hook enforces this). Never add MIT licensing to any other repo, and never add an open-source license to the private apps.
