...
- Licensing: public AGPL-3.0-only — `cli`, `console`, `coder-react`, `skills`, `multi-react`, `openbase-agent-ui-swift`, `openbase-agent-ui-kotlin`. Public MIT exceptions — `allauth-client-swift`, `allauth-client-kotlin`, `super-agents`, `boilersync-react`. Private/proprietary — `android`, `ios`, `netmesh-go`, and the netmesh companion sources (`desktop/netmesh-macos`, pending extraction to a private repo). `desktop` is transitioning to public source (Electron app plus the vendored Apache-2.0 screen-share companion) — see `desktop/OPEN_SOURCE_BOUNDARY.md`; do not add its license file or flip its visibility until that file's pre-flip checklist is complete. Never add MIT licensing to any other repo, and never add an open-source license to the private apps.
